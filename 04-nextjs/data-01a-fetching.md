...
## 🐛 Real-World Scenario: The E-Commerce Slowdown Crisis

### The Problem

**Company**: ShopFast, a mid-sized e-commerce platform
**Traffic**: 500,000 product page views/day
**Issue**: Page load times increased from 800ms to 4.5 seconds after Black Friday traffic spike

**Initial Metrics (Before Crisis):**
```
TTFB: 300-500ms
FCP: 800-1200ms
LCP: 1.2-1.8s
...
```
TTFB: 2.5-6s (10x increase!)
FCP: 3.5-7s
LCP: 4.5-9s
Bounce rate: 47% (4x increase!)
Conversion rate: 0.8% (75% drop!)
Error rate: 23% (timeout errors)
```

**Revenue Impact:**
- Lost sales: $150,000/day
- Support tickets: 2,400 complaints/day
- Brand damage: Trending on Twitter for slow performance
...
```typescript
// pages/products/[id].tsx - Using getServerSideProps
export async function getServerSideProps({ params, req }) {
  const productId = params.id;

  // Problem 1: Database query on every request
  const product = await db.product.findUnique({
    where: { id: productId },
    include: {
      reviews: { take: 10 },
      relatedProducts: { take: 5 },
      inventory: true
    }
  });

  // Problem 2: External API calls
  const recommendations = await fetch(
    `https://recommendations-api.shopfast.com/products/${productId}`
  ).then(r => r.json());

  // Problem 3: Slow pricing calculation
  const pricing = await calculateDynamicPricing(product, req.headers);
...
// Average execution time: 450ms per request
// At 100 requests/second: 100 concurrent executions
// Database connections: 100 simultaneous (hitting pool limit!)
```
...
1. **Database Connection Pool Exhaustion**:
   ```
   Max connections: 100
   Active connections during spike: 450
   Queue depth: 350 waiting requests
   Timeout errors: 23%
   ```

2. **Cold Start Cascade** (Vercel Serverless):
   ```
   Normal traffic: 20 warm functions
   Black Friday spike: 200 concurrent functions needed
   Cold starts: 180 functions (900ms each)
   Additional latency: +900ms per cold start
   ```

3. **External API Bottleneck**:
   ```
   Recommendations API response time: 150ms (normal)
   Recommendations API response time: 3.5s (spike)
   Their API was also overloaded!
   ```

4. **No Caching Strategy**:
   ```
   Same product fetched 10,000 times/hour
   95% of requests were duplicate data
   0% cache hit rate
   ```

### The Investigation Process

**Step 1: Monitoring Dashboard Analysis**

```bash
# Vercel Analytics showed:
- Function duration: p50=2.1s, p95=5.8s, p99=8.2s
- Error rate: 23% (mostly timeouts)
- Cold start rate: 85% (critical!)

# Database metrics (PostgreSQL):
- Connection pool utilization: 100%
- Query time: p50=1.2s (normally 45ms)
- Active queries: 450 (max 100)
- Deadlocks: 89/hour
```

**Step 2: APM Tracing (using Sentry/Datadog)**

```typescript
// Sentry trace revealed the waterfall:
Request → getServerSideProps (4.2s total)
  ├─ Database query (1.8s) ← SLOW!
  ├─ Recommendations API (2.1s) ← VERY SLOW!
  └─ Pricing calculation (0.3s)

// 95% of time spent waiting for I/O
```
...
-- Execution time: 1.8s (normally 15ms)
-- Reason: Missing index on deleted_at!
-- Rows scanned: 2.4M (full table scan on join)
```

**Step 4: Profiling Serverless Functions**
...
export async function getServerSideProps({ params }) {
  ...
  const product = await db.product.findUnique(...);
  console.timeEnd('db-query'); // 1842ms

  console.time('recommendations');
  const recommendations = await fetch(...);
  console.timeEnd('recommendations'); // 2134ms

  console.time('pricing');
  const pricing = await calculateDynamicPricing(...);
  console.timeEnd('pricing'); // 287ms

  console.timeEnd('total'); // 4263ms
}
```

### The Solution: Multi-Layered Optimization

**Phase 1: Immediate Mitigation (Deployed in 2 hours)**

```typescript
// 1. Switch to ISR with aggressive caching
export async function getStaticProps({ params }) {
  const product = await db.product.findUnique({
    where: { id: params.id },
    include: {
...
export async function getStaticPaths() {
  ...
  return {
    paths: topProducts.map(p => ({ params: { id: p.id } })),
    fallback: 'blocking' // Generate others on-demand, then cache
  };
}

// Result: 95% of requests served from cache
// TTFB: 50ms (was 2.5s) - 50x improvement!
```

**Phase 2: Optimize Remaining Dynamic Data (Deployed day 2)**

```typescript
// 2. Move non-critical data to client-side with SWR
'use client';

import useSWR from 'swr';

function ProductPage({ product }) {
  // Critical data: Server-rendered (SEO)
  // Non-critical data: Client-side (fast page load)

  const { data: recommendations } = useSWR(
    `/api/recommendations/${product.id}`,
    fetcher,
    {
      fallbackData: [], // Don't block render
      revalidateOnFocus: false
    }
  );
  ...
  const { data: inventory } = useSWR(
    ...
    fetcher,
    {
      ...
      fallbackData: product.inventory
    }
  );
...
**Phase 3: Database Optimization (Deployed day 3)**
...
-- Query time: 1.8s → 12ms (150x faster!)
```

**Phase 4: API Response Caching (Deployed day 4)**

```typescript
// api/recommendations/[id].ts
import { kv } from '@vercel/kv'; // Redis cache

export async function GET(req: Request, { params }) {
  const productId = params.id;

  // Check cache first
  const cached = await kv.get(`recommendations:${productId}`);
  if (cached) {
  ...
  // Cache for 5 minutes
  await kv.setex(`recommendations:${productId}`, 300, recommendations);

  return Response.json(recommendations);
}

// Result: 98% cache hit rate
// API response time: 3.5s → 45ms
```

**Phase 5: Implement Stale-While-Revalidate**
...
// Cache behavior:
// 0-60s: Serve cached (fresh)
// 60-360s: Serve cached (stale) + revalidate in background
// 360s+: Force revalidate
```
...
```
TTFB: 45ms (was 2.5s) - 55x improvement
FCP: 420ms (was 3.5s) - 8x improvement
LCP: 680ms (was 4.5s) - 6.6x improvement
Bounce rate: 9% (was 47%) - 81% reduction
Conversion rate: 3.8% (was 0.8%) - 375% increase!
Error rate: 0.02% (was 23%) - 1150x reduction
```

**Infrastructure Metrics:**
```
Database queries: 18,000/hour (was 500,000/hour) - 96% reduction
Serverless invocations: 12,000/hour (was 450,000/hour) - 97% reduction
CDN cache hit rate: 95% (was 0%)
Cold starts: 2% (was 85%)
```

**Business Impact:**
```
Revenue recovered: $150,000/day
Infrastructure cost: $2,400/day → $180/day (92.5% reduction)
Customer satisfaction: 4.8/5 (was 2.1/5)
Support tickets: 45/day (was 2,400/day)
```

### Lessons Learned

1. **Always have a caching strategy**: 95% of requests were serving duplicate data
2. **Monitor cold start rates**: High cold starts = need static rendering
3. **Database indexes matter**: Missing index caused 150x slowdown
4. **Don't fetch what you don't need immediately**: Move non-critical data client-side
5. **Set up proper monitoring**: Caught the issue 6 hours too late
6. **Load test before traffic spikes**: Black Friday should not have been a surprise
7. **Use ISR as default**: Switching from SSR to ISR saved the company
...
// ✅ Client-side for non-SEO critical data
useSWR(...)

// ❌ SSR only when absolutely necessary
// getServerSideProps() <- Avoid!
```

---

## ⚖️ Trade-offs: Choosing the Right Data Fetching Strategy

### Decision Matrix: When to Use Each Approach

| Strategy | Best For | Avoid For | TTFB | Cost | SEO | Freshness |
|----------|----------|-----------|------|------|-----|-----------|
| **SSG** | Blogs, docs, marketing | User-specific data | 20-50ms | $ | ⭐⭐⭐⭐⭐ | Hours-days |
| **ISR** | E-commerce, news | Real-time dashboards | 20-50ms | $$ | ⭐⭐⭐⭐⭐ | Seconds-minutes |
| **SSR** | Personalized dashboards | High traffic pages | 300-1800ms | $$$$ | ⭐⭐⭐⭐⭐ | Real-time |
| **CSR** | Admin panels, tools | Public SEO pages | 50-200ms | $ | ⭐ | Real-time |
...
**Advantages:**
- **Speed**: 20-50ms TTFB (served from CDN edge)
- **Cost**: Cheapest ($0.10 per 1M requests, pure CDN costs)
- **Scalability**: Infinite (no server execution)
- **Reliability**: No server dependencies, works during API outages
- **SEO**: Perfect (fully pre-rendered HTML)
- **Offline**: Can work offline with service worker

**Disadvantages:**
- **Build Time**: Long builds for large sites (10,000+ pages = 20+ min builds)
- **Stale Data**: Data is stale until next build or ISR revalidation
- **Dynamic Data**: Cannot show user-specific content
- **Preview**: Preview mode is complex for CMS workflows

**Cost Comparison (1M requests/month):**
```
SSG:
- CDN bandwidth: $0.10
- Build time: $0.05 (monthly rebuilds)
- Total: $0.15/month

SSR:
- Serverless execution: $15-25
- CDN bandwidth: $0.10
- Total: $15-25/month (100-167x more expensive!)
```
...
// - Marketing pages (same for all users)
// - About/Contact pages (pure content)
...
export async function getStaticProps() {
  ...
  return {
    props: { posts }
    // No revalidate: only update on rebuild
  };
}

// Build time: 2 minutes for 1000 pages
// Runtime cost: $0.10 per 1M requests
// User experience: 30ms page loads
```

**Anti-pattern:**
```typescript
// ❌ BAD: SSG for user-specific data
export async function getStaticProps() {
  // This is the SAME for all users!
  const user = await getCurrentUser(); // ← Wrong! No user context at build time
  return { props: { user } };
}
```
...
**Advantages:**
- **Speed**: Same as SSG (20-50ms) for cached pages
- **Fresh Data**: Periodic updates without full rebuilds
- **Scalability**: High (mostly CDN, occasional regeneration)
- **Flexibility**: Balance between static and dynamic
- **On-Demand**: Can trigger updates via webhook
- **SEO**: Perfect (pre-rendered HTML)

**Disadvantages:**
- **Stale Window**: First user after revalidate sees old content
- **Complexity**: Harder to reason about cache states
- **Regeneration Cost**: Background regenerations consume serverless time
- **Cache Warm-up**: First request after deploy can be slow
...
// Trade-off:
// - Freshness: Data can be up to 60s stale
// - Cost: Regenerate every 60s if traffic exists
// - Use for: Product listings, news articles

// Strategy 2: On-demand revalidation
export async function POST(request: Request) {
  const { productId } = await request.json();

  // CMS webhook triggers instant update
...
// Trade-off:
// - Freshness: Instant updates when content changes
// - Cost: Only regenerate when needed (cheaper)
// - Complexity: Need webhook infrastructure
// - Use for: CMS-driven content, inventory updates
...
// Later: revalidate all products at once
revalidateTag('products');

// Trade-off:
// - Freshness: Bulk updates
// - Cost: Regenerate multiple pages simultaneously
// - Use for: Category updates, global price changes
```

**Cost Analysis (E-commerce with 10,000 products):**

```
Scenario: 1M page views/month, 100 product updates/day

ISR with revalidate: 60s
- Cached requests: 950,000 ($0.10 CDN)
- Regenerations: 50,000 ($2.50 serverless)
- Total: $2.60/month

ISR with on-demand:
- Cached requests: 999,900 ($0.10 CDN)
- Regenerations: 100/day = 3,000/month ($0.15 serverless)
- Total: $0.25/month (10x cheaper!)

SSR:
- All requests: 1M ($25 serverless + $0.10 CDN)
- Total: $25.10/month (100x more expensive than on-demand ISR!)
```
...
// - News articles (publish new content)
// - User-generated content (comments, reviews)
// - Analytics dashboards (hourly updates)
...
export async function getStaticProps() {
  ...
  return {
    props: { product },
    revalidate: 300 // 5 minutes: balance freshness vs cost
  };
}

// Freshness: 5-minute stale max
// Cost: Regenerate every 5 min if traffic
// User experience: 30ms page loads (cached)
```
...
- **Request Context**: Access cookies, headers, IP address
- **User-Specific**: Can show personalized content
- **Real-Time**: Perfect for live dashboards
- **Authentication**: Easy to implement protected routes

**Disadvantages:**
- **Slow**: 300-1800ms TTFB (depends on data fetching)
- **Expensive**: 100-200x more costly than SSG/ISR
- **Scalability**: Limited by serverless concurrency
- **Cold Starts**: First request can be very slow (900ms+)
- **Database Load**: High (query on every request)

**Performance Characteristics:**

```typescript
// SSR execution breakdown
export async function getServerSideProps({ req, res }) {
  console.time('total');

  // Cold start (first request): 900ms
  // Warm start: 0ms
  ...
  const user = await db.user.findUnique({
    where: { id: getUserId(req.cookies.authToken) }
  });
  console.timeEnd('db-query'); // 234ms

  // External API: 100-1000ms
  console.time('api-call');
  const stats = await fetch('https://api.example.com/stats').then(r => r.json());
  console.timeEnd('api-call'); // 456ms

  console.timeEnd('total'); // 690ms (warm) or 1590ms (cold)

  return { props: { user, stats } };
}

// Total TTFB: 700-1600ms
// User perception: "Slow"
// Cost per 1M requests: $15-25
```

**Caching SSR Responses:**

You can mitigate some SSR costs with HTTP caching:
...
// Behavior:
// 0-10s: CDN serves cached response (fast!)
// 10-69s: CDN serves stale, revalidates in background
// 70s+: CDN fetches fresh from origin

// TTFB (cached): 50ms (CDN)
// TTFB (uncached): 800ms (origin)
// Cache hit rate: 85% (depends on traffic patterns)

// Cost reduction: 85% of requests served by CDN
// Effective cost: $2.25/1M requests (was $15)
```
...
export async function getServerSideProps({ req, res }) {
  ...
  return {
    props: { user, cartItems }
  };
}

// Freshness: Real-time (every request)
// Cost: $15-25 per 1M requests
// User experience: 700-1500ms page loads
// Use when: Data MUST be fresh AND user-specific
```

**Anti-pattern:**
```typescript
// ❌ BAD: SSR for content that could be static
export async function getServerSideProps() {
  const posts = await db.post.findMany();
...
// Problem:
// - Posts are same for all users
// - Could be SSG or ISR
// - Wasting 100x cost for no benefit!
```

#### 4. Client-Side Rendering (CSR)

**Advantages:**
- **Fast Initial Load**: Serve static HTML, fetch data after
- **Interactive**: Immediate page transitions
- **Cost-Effective**: No serverless execution for page render
- **Real-Time**: Easy to poll/subscribe for updates
- **User-Specific**: Data tied to logged-in user

**Disadvantages:**
- **Poor SEO**: Content not in initial HTML
- **Slow FCP**: "Loading..." state visible to users
- **Multiple Requests**: Waterfall loading (HTML → JS → API → Data)
- **No SSR Benefits**: Loses pre-rendering advantages
...
// - Interactive tools (calculators, editors)
...
// SEO: None (admin panel, OK to skip)
// Freshness: Real-time (5s polling)
// Cost: API route execution only ($0.50 per 1M)
// User experience: Fast page load + loading states
```
...
Combine multiple strategies in a single page:

```typescript
// 1. Critical content: SSG/ISR (SEO, fast load)
export async function getStaticProps({ params }) {
  const product = await db.product.findUnique({
    where: { id: params.id }
  });
...
// 2. Non-critical content: Client-side (SWR)
'use client';

function ProductPage({ product }) {
  // Critical: Server-rendered (SEO)
  const productHTML = renderProduct(product);

  // Non-critical: Client-side (fast page load)
  const { data: reviews } = useSWR(`/api/reviews/${product.id}`, fetcher);
  const { data: inventory } = useSWR(`/api/inventory/${product.id}`, fetcher);
  const { data: recommendations } = useSWR(`/api/recommendations/${product.id}`, fetcher);

  return (
    <div>
      {productHTML}
      <Reviews data={reviews} /> {/* Loading state OK */}
      <Inventory data={inventory} /> {/* Updates every 30s */}
      <Recommendations data={recommendations} /> {/* Non-critical */}
    </div>
  );
}

// Result:
// - FCP: 400ms (product visible immediately)
// - LCP: 600ms (hero image loaded)
// - SEO: Perfect (product data in HTML)
// - Cost: $0.20 per 1M requests (mostly CDN)
// - Freshness: Product (60s), Reviews/Inventory (real-time)
```

### The Ultimate Decision Tree

```
START: Do you need this data?
  │
  ├─ For SEO? (Googlebot must see it)
  │  │
  │  ├─ YES → Server-render it (SSG/ISR/SSR)
  │  │  │
  │  │  ├─ Data changes?
  │  │  │  ├─ Never/Rarely → SSG
  │  │  │  ├─ Periodically (minutes/hours) → ISR
  │  │  │  └─ Every request (user-specific) → SSR
  │  │
  │  └─ NO → Client-side render (CSR)
  │     └─ Use SWR/React Query
  │
  └─ Is it user-specific?
     │
     ├─ YES → SSR or CSR
     │  │
     │  ├─ High traffic? → CSR (cheaper)
     │  └─ Low traffic? → SSR (simpler)
     │
     └─ NO → SSG or ISR
        │
        ├─ Updates < 1/hour → SSG
...
## 💬 Explain to Junior: Data Fetching Strategies Simplified

### The Restaurant Analogy

Imagine you're running a restaurant. How you serve food to customers is like how Next.js fetches and serves data to users.

#### 1. Static Site Generation (SSG) = Meal Prep Service

**How it works:**
You cook all the meals on Sunday (build time), package them, and store them in customers' fridges (CDN). When customers are hungry (request a page), they just microwave it (serve from CDN) - instant!

**Real code:**
```typescript
// Like cooking all meals on Sunday
export async function getStaticProps() {
  const recipes = await cookbook.getAllRecipes(); // Cook once

  return {
    props: { recipes } // Package and store
  };
}

// When customer requests: Just serve from storage (CDN)
// Time: 30 seconds (microwave time)
// Cost: Super cheap (no chef needed)
```

**When to use:**
- Menu (your website's navigation)
- Recipe blog (content doesn't change much)
- Restaurant brochure (marketing pages)

**When NOT to use:**
- Personalized meal plan (user-specific data)
- Today's special based on your diet (dynamic content)

#### 2. Incremental Static Regeneration (ISR) = Buffet with Fresh Refills

**How it works:**
You prepare food in advance (build time), but every 30 minutes, you check each tray. If it's been sitting for a while, you make a fresh batch and replace it. Customers always get hot, fresh food, but you don't cook to order.

**Real code:**
```typescript
// Like preparing buffet trays
export async function getStaticProps() {
  const todaysSpecials = await kitchen.getTodaysMenu();

  return {
    props: { todaysSpecials },
    revalidate: 60 // Check tray every 60 seconds
  };
}

// Customer flow:
// 1. Customer arrives → Serve from buffet (fast!)
// 2. After 60s → Check if food is fresh
// 3. If stale → Replace tray in background
// 4. Next customer → Gets fresh food
```

**When to use:**
- Product pages (prices change occasionally)
- News site (new articles every hour)
- E-commerce (inventory updates)

**The "stale" problem:**
- First customer after 60s gets old food
- But it's still good (just not fresh-fresh)
- Next customer gets the new batch

#### 3. Server-Side Rendering (SSR) = Cook to Order

**How it works:**
You start cooking when the customer orders (every request). They wait 10-15 minutes, but the meal is perfectly fresh and customized to their preferences.

**Real code:**
```typescript
// Like taking a custom order
export async function getServerSideProps({ req }) {
  const customerId = req.cookies.userId;

  // Check their dietary restrictions
  const preferences = await getUserPreferences(customerId);

  // Cook a fresh meal based on their needs
  const personalizedMeal = await kitchen.cookFresh(preferences);

  return {
    props: { personalizedMeal }
  };
}

// Customer flow:
// 1. Customer orders → Chef starts cooking
// 2. Wait 10-15 min → Food is being prepared
// 3. Serve → Hot, fresh, personalized!
```

**When to use:**
- Personal meal plan dashboard (user-specific)
- Live order tracking (real-time data)
- Dietician consultation (authentication required)

**When NOT to use:**
- Menu (same for everyone, waste of chef time)
- Recipes (cook-to-order for static content = silly!)

#### 4. Client-Side Rendering (CSR) = Self-Service Salad Bar

**How it works:**
You serve the customer an empty plate (HTML), then they go to the salad bar and build their own salad (fetch data client-side). Fast to get started, but they have to do some work.

**Real code:**
```typescript
// Like serving an empty plate
'use client';

import useSWR from 'swr';

function SaladBar() {
  const [plate, setPlate] = useState([]); // Empty plate

  // Customer walks to salad bar and picks items
  const { data: veggies } = useSWR('/api/vegetables', fetcher);
  const { data: dressings } = useSWR('/api/dressings', fetcher);

  if (!veggies) return <EmptyPlate />; // Walking to salad bar...

  return (
    <div>
      <Veggies items={veggies} />
      <Dressings items={dressings} />
    </div>
  );
}
```
...
**When to use:**
- Admin panel (customer is chef, doesn't need menu SEO)
- Live dashboard (updates every few seconds)
- Interactive configurator (lots of real-time changes)

**When NOT to use:**
- Public menu (Google can't see the salad bar)
- Marketing page (first impression matters)

### The Performance Comparison (In Restaurant Terms)

```
SSG (Meal Prep):
  Time to table: 30 seconds (just microwave)
  Cost: $1 per meal (pre-cooked)
  Freshness: 1-7 days old (but still good)
  Scalability: Feed 10,000 people (just need microwaves)

ISR (Buffet):
  Time to table: 1 minute (buffet line)
  Cost: $3 per meal (occasional refill)
  Freshness: 0-30 minutes old (pretty fresh)
  Scalability: Feed 5,000 people (refill as needed)

SSR (Cook to Order):
  Time to table: 15 minutes (wait for cooking)
  Cost: $25 per meal (chef time + ingredients)
  Freshness: 0 seconds old (perfectly fresh)
  Scalability: Feed 100 people (limited by chef capacity)

CSR (Salad Bar):
  Time to table: 5 minutes (self-service)
  Cost: $2 per meal (ingredients only)
  Freshness: Real-time (build it yourself)
  Scalability: Feed 1,000 people (self-service scales)
```
...
```typescript
// ❌ Junior code: "I'll just use SSR everywhere, it's simple!"
export async function getServerSideProps() {
  const posts = await db.post.findMany();
  return { props: { posts } };
}

// Problem: Like cooking-to-order for a menu
// - Menu is same for everyone
// - No need to cook fresh for each customer
// - Wastes chef time (serverless execution)
```
...
// Now: Pre-made meal (30 seconds vs 15 minutes)
```

#### Mistake 2: Using Client-Side for SEO Content

```typescript
// ❌ Junior code: "I'll fetch it with useEffect, clean and simple!"
'use client';
...
// Problem: Like serving empty plates to health inspectors
// - Google sees empty plate (no content)
// - Customers see "Loading..." first (bad UX)
// - SEO score: 0/100
```
...
// Now: Customers see content immediately, Google is happy
```

#### Mistake 3: Not Using ISR for Dynamic Content

```typescript
// ❌ Junior code: "Content changes sometimes, so SSR!"
export async function getServerSideProps() {
  const product = await db.product.findUnique(...);
  return { props: { product } };
}

// Problem: Like cooking-to-order for a popular dish
// - Same burger cooked 1,000 times today
// - Could have batch-cooked and reheated
// - Waste of chef time and money
```
...
// Now: 1,000 customers served in 30 seconds (not 15 minutes)
// Fresh enough (5-minute-old burger is fine)
```

### Interview Answer Template

**Question: "When would you use getStaticProps vs getServerSideProps?"**

**Perfect Junior Answer:**

"I'd use **getStaticProps (SSG)** for content that's the same for all users and doesn't change often, like blog posts, documentation, or marketing pages. It's the fastest because pages are pre-built and served from a CDN.

For content that updates periodically, I'd add **revalidate (ISR)** to getStaticProps, like `revalidate: 60` for product pages that update every minute. This gives me fast page loads but with fresh data.

I'd only use **getServerSideProps (SSR)** for user-specific or real-time data where I need access to the request object, like dashboards, authentication-protected pages, or data that must be fresh on every request. But I'd try to avoid SSR when possible because it's slower and more expensive.

For non-SEO-critical data, I'd use **client-side fetching with SWR**, like admin panels or real-time dashboards where SEO doesn't matter and I want real-time updates."

**Why this answer works:**
1. Shows understanding of all approaches
2. Explains the "why" not just the "what"
3. Mentions performance and cost considerations
4. Demonstrates real-world thinking
...
Need SEO?
  ↓
YES → Server-render (SSG/ISR/SSR)
  ├─ Static content → SSG
  ├─ Updates periodically → ISR
  └─ User-specific → SSR

NO → Client-render (CSR)
  └─ Use SWR/React Query

Too slow?
  └─ Move non-critical data to client-side

Too expensive?
  └─ Switch SSR to ISR or SSG

Too stale?
  └─ Reduce revalidate time or use SSR
```
